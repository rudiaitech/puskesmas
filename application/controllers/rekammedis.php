...
class Rekammedis extends CI_Controller {
	...
	public function index()
	{
		$data = array(
			...
			'judul'			=> 'Rekam Medis',
			'data' 			=> $this->pelayanan->tabel_bydokter()->result(),
			'content'		=> 'rekammedis/v_content',
	...
	public function update()
	{
		...
		array( 'required'  => '%s harus diisi!'));
		$this->form_validation->set_rules('pemeriksaan_dokter', 'Pemeriksaan Dokter', 'required',
		array( 'required'  => '%s harus diisi!'));
		$this->form_validation->set_rules('diagnosa_dokter', 'diagnosa Dokter', 'required',
		array( 'required'  => '%s harus diisi!'));
		$this->form_validation->set_rules('tindakan_dokter', 'tindakan Dokter', 'required',
		array( 'required'  => '%s harus diisi!'));

		if (!$this->form_validation->run()) 
		{
			$this->session->set_flashdata('error', '<i class="fa fa-warning"></i> Peringatan! Pastikan Data Sudah Diisi Dengan Benar');
			redirect(base_url('rekammedis/rekam'),'refresh');
		}
		...
		if($cek == null){
			...
			redirect(base_url('rekammedis'),'refresh');
				...
				$data = array(
					...
					'pemeriksaan_dokter'	=> $this->input->post('pemeriksaan_dokter'),
	...
	{
		$dokter = $this->dokter->list($id)->result();
