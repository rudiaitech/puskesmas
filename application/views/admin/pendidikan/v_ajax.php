<!-- Data table plugin-->
	...
	<script>
		$(document).on("click",".btnedit",function(){
			...
			$.ajax(
			{
				url : "<?= base_url()?>admin/pendidikan/edit/"+id,
				type: "GET",
				dataType : "JSON",
				success: function(data)
				{
					$('#e_id_pendidikan').val(data.id_pendidikan);
					$('#e_nama_pendidikan').val(data.nama_pendidikan);
		...
		$(document).on("click",".btnhapus",function(){
			...
			closeOnConfirm: true
				...
				if (isConfirm) {
						window.location = "<?= base_url() ?>admin/pendidikan/delete/"+id;
